...
/-!
# Green's Open Problem 25: standalone Lean4Web proof

This file reproduces the current Formal Conjectures definitions of Green 25 and
proves the improved upper-bound target with the explicit color count
`N ↦ ⌈N ^ (23 / 40)⌉`.  It uses mathlib only.
-/
...
/-- The restricted sumset `{s₁ + s₂ : s₁, s₂ ∈ S, s₁ ≠ s₂}`
...
/-- `FormalConjectures/GreensOpenProblems/25.lean`. -/
...
/- ## `answer( )` from Formal Conjectures

Copied from `FormalConjecturesUtil/Answer/Syntax.lean` and
`FormalConjecturesUtil/Answer.lean` (Apache-2.0, The Formal Conjectures Authors),
dropping only the module-system keywords (`module`, `public`, `meta`).
Lean4Web checks a single file, so the option `google.answer` is replaced by its
default value `.alwaysTrue`. -/
...
/-- Lean4Web change: an option cannot be registered and read in the same file,
so the default value of `google.answer` is fixed here. -/
def answerSetting : AnswerSetting := .alwaysTrue
...
theorem block_bound_with_remainder (L k r : ℕ) (q a b : ℝ)
  ...
  have hBr : blockCoeff r q ≤ (2 * (3 / 2 : ℝ) ^ q) ^ r := by
    have h := hr
    simp only [Nat.one_mul, blockCoeff_one, show (1 / 2 : ℝ) + 1 / 2 = 1 by norm_num,
...
theorem exists_coloring_with_few_bad_sums (d t B : ℕ)
  ...
  let : DecidableRel bad := fun _ _ => Classical.propDecidable _
  have hcols :
      (∑ s : CandidateSum d,
...
theorem ternaryValue_eq_carryValue_iff {d : ℕ} (z : Fin d → Fin 3)
  ...
  rw [show bitNat false = 0 from rfl, add_zero] at hv
  constructor
  · intro h
...
theorem block8_coefficient_bound :
  ...
  calc
    ...
    _ = (312445416 : ℝ) / 1000000 := by
      simpa [List.map_map, Function.comp_def, root_sum] using scaled_nat_sum ((terminalNorms 8 1 1).map rootUpper)
  ...
  have hraw_refines :
    ...
    · by_cases hy : y ∈ Set.range pick
      · have hi : Classical.choose hx = Classical.choose hy := by
          ...
          simpa only [raw, dite_eq_left hx, dite_eq_left hy] using hxy
        have hpoint : x = y :=
      ...
      · have hbad : Sum.inr (Classical.choose hx) = Sum.inl (q y) := by
          simpa only [raw, dite_eq_left hx, dite_eq_right hy] using hxy
        exact False.elim (Sum.inr_ne_inl hbad)
    · by_cases hy : y ∈ Set.range pick
      · have hbad : Sum.inl (q x) = Sum.inr (Classical.choose hy) := by
          simpa only [raw, dite_eq_right hx, dite_eq_left hy] using hxy
        exact False.elim (Sum.inl_ne_inr hbad)
      · have hqxy : q x = q y := by
          apply Sum.inl_injective
          simpa only [raw, dite_eq_right hx, dite_eq_right hy] using hxy
...
/-- The body of `Green25.green_25.upper` with the explicit answer
`N ↦ ⌈N ^ (23 / 40)⌉`. -/
theorem green_25_upper :
  ...
  green_25_upper_concrete

end Green25Fractional

#print axioms Green25Fractional.green_25_upper
